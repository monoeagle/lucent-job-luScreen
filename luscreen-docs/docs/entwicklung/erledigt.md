...
### Politur 0.2.1 — Verlauf-UX + Tray-Lesbarkeit — abgeschlossen `20260516-2210`

- [x] **Löschen-Button ans Ende der Toolbar** mit eigenem Trenner davor (`src/views/history-window.xaml`). Reihenfolge jetzt: Anzeigen / Bearbeiten / Umbenennen / Speicherort / Clipboard / Liste / Refresh · `|` · Druck · `|` · Löschen. Reduziert ungewolltes Löschen beim Greifen nach den Lese-Aktionen.
- [x] **Single-Instance für Tray-Dialoge** (Verlauf / Konfiguration / Über). Vorher öffnete ein zweiter Tray-Klick — während `ShowDialog` in seinem nested Dispatcher pumpte — eine weitere Instanz übereinander. Jetzt prüft jedes `Show-*` einen `$script:OpenWindow`-Slot und holt das bestehende Fenster per `Topmost`-Toggle + `Activate` in den Vordergrund. Slot wird beim `Closed`-Event genullt. **Wichtig:** `$script:OpenWindow = $null` muss auf Modul-Ebene initialisiert werden, sonst killt `Set-StrictMode -Version Latest` den allerersten Zugriff (`Variable nicht festgelegt`).
- [x] **Tray-Icon farblich invertiert** (`_New-TrayIcon -Invert` in `src/ui/tray.psm1`). ICO/PNG → `Bitmap.ToBitmap()` → `ColorMatrix` mit `Matrix00/11/22 = -1` und `Matrix40/41/42 = 1` (RGB-Inversion, Alpha bleibt) → `GetHicon()` → `Icon.FromHandle`. `Initialize-Tray` nimmt `-InvertIcon` (Default `$true`). Damit ist die schwarze Glyph-Optik auf dunkler Taskleiste lesbar.
- [x] **Schloss pro Verlaufs-Bild.** Neue `IsLocked`-Property an der C#-Klasse `LucentScreen.HistoryEntry` (mit `INotifyPropertyChanged`); Quelle ist das NTFS-`ReadOnly`-Attribut. `New-HistoryItem` in `src/core/history.psm1` exportiert `IsReadOnly`, das XAML-`DataTemplate` rendert einen `Button x:Name="BtnLock"` rechts oben über dem Thumbnail. Style mit `DataTrigger` schaltet Glyph (E785 offen / E72E zu) + Foreground/Background/Border zwischen Grün (`#15803D` auf `#CCDCFCE7`) und Rot (`#B91C1C` auf `#CCFEE2E2`). Click-Routing: `LstItems.AddHandler(Button.ClickEvent, ...)` — bubblender Routed-Event vermeidet Sub-Klick-Probleme bei `ListBoxItem.IsSelected`. `cmdDelete` filtert gesperrte Einträge raus, meldet Skips im Confirm-Text. Keine Sidecar-Datei, kein zusätzliches Schema-Migrationspaar.

**Artefakte:**
- `src/views/history-window.xaml` — neue `LockButton`-Style mit `DataTrigger`; ItemTemplate-Grid mit `BtnLock`-Overlay; Toolbar-Reihenfolge umgestellt.
- `src/ui/history-window.psm1` — `HistoryEntry` erweitert, `$mkEntry` setzt `IsLocked`, `cmdDelete` skippt gesperrte, neuer Lock-Toggle-Routed-Handler; `$script:OpenWindow` initialisiert + Single-Instance-Guard.
- `src/core/history.psm1` — `New-HistoryItem` exportiert `IsReadOnly`.
- `src/ui/tray.psm1` — `_New-TrayIcon -Invert`-Pfad mit `ColorMatrix`; `Initialize-Tray -InvertIcon` (Default true).
- `src/ui/config-dialog.psm1`, `src/ui/about-dialog.psm1` — Single-Instance-Guard + `$script:OpenWindow`-Init.

**Implementierungs-Detail:**
- **StrictMode-Trap:** Beim ersten Tray-Klick auf Verlauf warf das Log `Die Variable "$script:OpenWindow" kann nicht abgerufen werden, weil sie nicht festgelegt wurde.` Fix: `$script:OpenWindow = $null` direkt nach `Set-StrictMode -Version Latest`, vor allen Funktionsdefinitionen.
- **ColorMatrix-Inversion mit Alpha-Erhalt:** Die identity-Matrix wäre `Matrix00..44 = 1`. Wir negieren RGB-Diagonale und schieben dann pro Kanal um `+1` zurück in den positiven Bereich (`Matrix40/41/42 = 1`) — entspricht `c' = 1 - c`. `Matrix33 = 1` (Alpha) bleibt unverändert, sonst würde Transparenz auch invertiert.
- **Lock via ReadOnly:** Statt einer Sidecar-Datei oder neuen Config-Schemas: das NTFS-Attribut. Vorteile: persistent, kein Migrations-Aufwand, Explorer sieht es auch. Nachteil: „Löschen im Dateisystem ist nicht abgesichert" — was der User explizit als OK markiert hat.
...
| 33 | `20260516-2210` | `_pending_` | — | polish | **0.2.1**: Verlauf-Loeschen ans Ende der Toolbar + Trenner. Single-Instance-Guard fuer Verlauf-/Konfig-/About-Dialog (Tray-Klick holt bestehendes Fenster nach vorne statt zweites zu oeffnen). Tray-Icon-Inversion (RGB via ColorMatrix, Alpha bleibt) -- Default-on via `Initialize-Tray -InvertIcon`. Schloss-Toggle pro Verlaufs-Bild ueber NTFS-ReadOnly (gruen offen / rot zu, mit Alpha-Hintergrund); `cmdDelete` skippt gesperrte. `$script:OpenWindow`-Slot in allen drei Dialog-Modulen vorinitialisiert -- StrictMode-Latest verbot sonst den ersten Zugriff. `LucentScreen.ps1` `$appVersion = '0.2.1'`. CHANGELOG + Verlauf-Anleitung + Entwicklungs-Changelog + Erledigt aktualisiert. Bundle `LucentScreen_v0.2.1` neu erzeugt (gitignored). |
